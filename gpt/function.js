...
(async () => {
  ...
  const response = await openai.createChatCompletion(gptBody);
  if (response.data.choices[0].finish_reason == "function_call") {
    if (response.data.choices[0].message.function_call.name == "get_names") {
      const function_response = get_names(parseInt(JSON.parse(response.data.choices[0].message.function_call.arguments).num));
      gptBody.messages.push({
        role: "function",
        name: "get_names",
        content: JSON.stringify(function_response)
      });
      const response2 = await openai.createChatCompletion(gptBody);
      saveOutput('function', response2);  
    } else {
      console.log("gpt wants another function: " + response.data.choices[0].message.function_call.name);
    }
  } else {
    console.log("gpt does not want a function");
  }
})();